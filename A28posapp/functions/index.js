...
exports.onSaleCreated = onDocumentCreated({
    ...
    // 2. Atomic Posting to Individual Account Period Vectors
    for (const leg of jvEntries) {
        const coaInfo = COA_MASTER_TYPES[leg.accountCode] || { name: leg.accountName, type: 'EXPENSE' };
        const periodVectorDocRef = db.doc(`users/${ownerId}/shops/${shopId}/account_periods/${keys.monthKey}_${leg.accountCode}`);
        
        batch.set(periodVectorDocRef, {
