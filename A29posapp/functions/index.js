...
// =========================================================================
async function assertAccountingPeriodUnlocked(tenantId, shopId, isoDateStr) {
    ...
    if (periodLockSnap.exists && periodLockSnap.data().isLocked === true) {
        console.error(`🚨 TEMPORAL SECURITY VIOLATION: Transaction date ${isoDateStr} falls into LOCKED period ${periodKey}!`);
...
// =========================================================================
// 🏛️ SRI LANKA IRD RAMIS UNIVERSAL DUAL TIN/VAT PARSER (GAZETTE COMPLIANT)
// පැරණි 9-Digit සහ නව 12-Digit Digital TIN මෙන්ම 13/16-Digit VAT නිරවුල් කිරීම
// =========================================================================
function sanitizeAndValidateSriLankanTin(rawTin, isMandatory = false) {
    ...
    if (isMandatory) {
        throw new Error(`INVALID_TIN_FORMAT: '${rawTin}' is invalid. Sri Lankan TIN must be 9 digits (standard) or 12 digits (digital series).`);
    }
...
exports.pushInvoiceToRamis = onCall({ timeoutSeconds: 30, memory: "512MiB" }, async (request) => {
    ...
    const authoritativePayload = {
        ...
        buyerTin: (verifiedSale.customer && verifiedSale.customer.tinNumber) ? verifiedSale.customer.tinNumber : null,
...
exports.onSaleCreated = onDocumentCreated({
    ...
    // =========================================================================
    // 🛡️ FORENSIC FALLBACK: බිල්පත Formal Invoice එකක් නම් හෝ පැතිකඩෙහි Tax Inclusive සක්‍රීය නම් නිවැරදිව හඳුනාගැනීම
    let isTaxInclusive = sale.isTaxInclusive === true;
    const claimedTaxAmt = parseFloat(sale.vatAmt !== undefined ? sale.vatAmt : (sale.taxAmt || 0)) || 0;

    // සැබෑ බිලෙහි total අගය බදු එකතු නොකළ අගයට ගැලපේ නම්, එය ස්වයංක්‍රීයව Tax-Inclusive ලෙස නිරවුල් කිරීම
    const netBaseWithoutTax = (sale.subtotal || 0) - (sale.discountAmt || 0) + (sale.feesTotal || 0) + (sale.surchargeAmt || 0) - (sale.loyaltyRedeemed || 0);
    if (!isTaxInclusive && Math.abs(netBaseWithoutTax - (sale.total || 0)) <= 1.0 && claimedTaxAmt > 0) {
        isTaxInclusive = true;
    }
    ...
    let expectedTotal = (sale.subtotal || 0) 
                      - (sale.discountAmt || 0) 
                      + (sale.feesTotal || 0) 
                      + (isTaxInclusive ? 0 : claimedTaxAmt) 
                      + (sale.surchargeAmt || 0) 
    ...
    // =========================================================================
    // 🏛️ ZERO-TRUST PROMO-AWARE AUTHORITATIVE PRICE AUDITOR (ANTI-TAMPERING)
    // Buy Cost හි 5% සීමාව ඉවත් කර, නීත්‍යානුකූල Base Price & Active Promos සමඟ සැසඳීම
    // =========================================================================
        ...
        try {
            ...
            for (let idx = 0; idx < productSnapshots.length; idx++) {
                ...
                if (pSnap.exists) {
                    ...
                    // 1. Backend එක මඟින් අවසරලත් අවම නීත්‍යානුකූල මිල ස්වාධීනව ගණනය කිරීම
                    let legalFloorPrice = dbBaseSell;

                    // A. Time-bound Promo එකක් ක්‍රියාත්මකව තිබුණේදැයි බැලීම
                    if (pData.promoEnd && pData.promoValue > 0) {
                        const promoEndMs = new Date(pData.promoEnd).getTime();
                        if (promoEndMs >= saleTimeMs) {
                    ...
                    // B. පරිපාලක ස්ථිර වට්ටම් පරීක්ෂාව
                    ...
                    // 2. Cart Level Global Discount සහන සීමාව (Allow up to 35% global basket discount variance)
                    // පාරිභෝගිකයාට මුළු බිලටම දුන් Discount එක නිසා මිල තවදුරටත් අඩුවිය හැකි අවම සීමාව:
                    const absoluteBottomLimit = Math.max(dbBuyCost * 0.85, legalFloorPrice * 0.65);

                    // 🚨 සැබෑ මිල විකෘති කිරීම හසුකර ගැනීම (Claims below legal promotional floor)
    ...
    // =========================================================================
    const rollupShardIdx = Math.floor(Math.random() * 20); // 0 සිට 19 දක්වා Shards 20ක්
    ...
    // Balance Sheet එකද Shards 20 දක්වා පුළුල් කිරීම
    const bsShardIdx = Math.floor(Math.random() * 20);
    ...
    if (isRamisReportable) {
        ...
        const cleanBuyerTin = (sale.customer && sale.customer.tinNumber) ? String(sale.customer.tinNumber).replace(/[^0-9]/g, '').substring(0, 9) : null;
        ...
        batch.set(outboxDocRef, {
            ...
            buyerTin: cleanBuyerTin, // 👈 🏛️ 100% Preserved 9-Digit Purchaser TIN for B2B Input VAT
        ...
        if (multiplier === 1) { // Newly created or updated active voucher
            ...
            if (exp.isIncome) {
                ...
                if (exp.isCapex) {
                    ...
                    addExpLeg('6010', 'Staff Payroll Expense', grossExpenseAmt, 0);
                } else {
...
exports.onChequeUpdated = onDocumentUpdated({
    ...
    if (after.status === 'CLEARED') {
        ...
        let settledDebt = 0;
        if (after.breakdown && typeof after.breakdown === 'object') {
            Object.values(after.breakdown).forEach(val => {
...
exports.biometricPushWebhook = onRequest({ cors: true }, async (req, res) => {
    ...
    try {
        ...
        if (isClockIn) {
            ...
            const shiftRoster = shiftRosterSnap.exists() ? shiftRosterSnap.data().days[shiftDateStr] : null;
...
exports.generateBankUnderwritingReport = onCall({ timeoutSeconds: 30, memory: "512MiB" }, async (request) => {
    ...
    try {
        ...
        // =========================================================================
        // 🏛️ TARGETED STATUS QUERY (CLOUD FUNCTION OOM CRASH TERMINATED)
        // සියලුම චෙක්පත් කියවීම වෙනුවට Bounced වූ චෙක්පත් පමණක් (Exact Query) කියවීම
        // =========================================================================
        const bouncedChqQ = await db.collection(`users/${ownerId}/shops/${shopId}/issued_cheques`)
...
exports.closeAccountingPeriod = onCall({ timeoutSeconds: 60, memory: "1GiB" }, async (request) => {
    ...
    // 2. Gather All Account Period Vectors to produce the Official Trial Balance
    ...
    const tbPromises = coaMasterCodes.map(code => 
        db.doc(`users/${tenantId}/shops/${shopId}/account_periods/${periodKey}_${code}`).get()
    );
    const tbSnaps = await Promise.all(tbPromises);
    ...
    tbSnaps.forEach(s => {
        if (s.exists) {
            const d = s.data();
            const dr = secureRound(d.totalDebit || 0);
            const cr = secureRound(d.totalCredit || 0);
            if (dr > 0 || cr > 0) {
    ...
    const sealPayload = `${periodKey}_${netRevenue}_${netProfit}_${totalTbDr}_${totalTbCr}_${balanceSheet.cashInHand}_${Date.now()}`;
    ...
    const snapshotData = {
        ...
        taxLiabilities: {
            ...
            netVatPayable: secureRound(pnl.vatCollected - pnl.taxReversed)
        }
    };
...
exports.nightlyFinancialReconciliationEngine = onSchedule({
    ...
    try {
        const [yesterdaySalesSnap, yesterdayExpensesSnap] = await Promise.all([
            ...
            try {
                ...
                expensesSnap.forEach(eDoc => {
                    ...
                    if (exp.isIncome) {
                        ...
                        if (exp.isPayroll) actualPayroll += amt;
                ...
                console.error(`❌ [RECONCILIATION CRASH] Failed reconciling tenant shop ${tenantShopKey}:`, shopErr.message);
            }
        }

        console.log(`🏁 [RECONCILIATION FINISHED] Audited ${activeShopKeys.size} active shops. Total Healed: ${totalShopsHealed}. Total Discrepancies: ${totalDiscrepanciesDetected}.`);
