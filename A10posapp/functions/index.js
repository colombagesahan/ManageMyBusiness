...
// ------------------------------------------------------------------------
// 1. TRIGGER: ON SALE CREATED (Includes F&B COGS Fix)
// ------------------------------------------------------------------------
exports.onSaleCreated = onDocumentCreated("users/{ownerId}/shops/{shopId}/sales/{saleId}", async (event) => {
    ...
    // 🚨 CQRS FIX: Calculate Standard COGS + F&B Recipe COGS
    let cogs = sale.recipeCogs || 0; 
    if (sale.items && Array.isArray(sale.items)) {
        sale.items.forEach(i => { 
            // Services and Recipe-items might have buy=0, which is fine.
            cogs += (parseFloat(i.buy) || 0) * (parseFloat(i.qty) || 1); 
        });
    }
    ...
    const discounts = (sale.discountAmt || 0) + (sale.loyaltyRedeemed || 0);

    const rollupPayload = {
        grossSales: FieldValue.increment(secureRound(gross)),
        totalDiscounts: FieldValue.increment(secureRound(discounts)),
    ...
    const glPayload = {
        ...
        // 🚨 IFRS FIX: Capture Customer Cheque as an Asset!
...
exports.onExpenseWritten = onDocumentWritten("users/{ownerId}/shops/{shopId}/expenses/{expenseId}", async (event) => {
    ...
    const applyExpenseImpact = (exp, multiplier) => {
        ...
        if (exp.isAdvanceEvent) {
            ...
            } else if (exp.description && exp.description.includes('Debt Settled') && exp.amount > 0) {
...
exports.onRefundCreated = onDocumentCreated("users/{ownerId}/shops/{shopId}/refunds/{refundId}", async (event) => {
    ...
    const cogsReversed = r.restock ? secureRound((r.amount - (r.profitReversal || 0))) : 0;
...
exports.onProductWritten = onDocumentWritten("users/{ownerId}/shops/{shopId}/products/{productId}", async (event) => {
    ...
    const oldAssetValue = secureRound(oldQty * oldBuy);
    const newAssetValue = secureRound(newQty * newBuy);
    
    const assetDelta = newAssetValue - oldAssetValue;
    let payablesDelta = 0;

    // Only recognize Accounts Payable on BRAND NEW Credit or Cheque Purchases
            if (!before && after && (after.payStatus === 'Credit' || after.payStatus === 'Cheque')) {
                payablesDelta = newAssetValue; 
            }
