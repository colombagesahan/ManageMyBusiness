...
exports.onExpenseWritten = onDocumentWritten({
    ...
    // 🛡️ IDEMPOTENCY RETRY FIREWALL: Suppress duplicate GCF invocations of the same event
...
exports.generateBankUnderwritingReport = onCall({ timeoutSeconds: 30, memory: "512MiB" }, async (request) => {
    ...
    try {
        ...
        // 2. Extract 6-Month Macro Financial Telemetry from Distributed Sharded CQRS Rollups
        const monthKeys = [];
        const slDateNow = new Date(serverNowMs + (5.5 * 60 * 60 * 1000));
        let curY = slDateNow.getUTCFullYear();
        let curM = slDateNow.getUTCMonth() + 1; // 1 to 12
...
exports.closeAccountingPeriod = onCall({ timeoutSeconds: 60, memory: "1GiB" }, async (request) => {
    ...
    // 🛡️ RECONCILED TEMPORAL ORDER: Executed strictly AFTER Tax & Asset Reclassifications
    const sealPayload = `${periodKey}_${netRevenue}_${netProfit}_${totalTbDr}_${totalTbCr}_${balanceSheet.cashInHand}_${statutoryNetVatPayable}_${excessTaxCreditCarriedForward}_${Date.now()}`;
