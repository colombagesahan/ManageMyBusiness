...
exports.onExpenseWritten = onDocumentWritten({
    ...
    const applyExpenseImpact = (exp, multiplier) => {
        ...
        // 🚀 DISTRIBUTED SHARDED COUNTER ENGINE (HOTSPOTTING CRASH ELIMINATED)
...
exports.onRefundCreated = onDocumentCreated({
    ...
    // 1. Rollups Update (Naturally sharded by Date)
    batch.set(db.doc(`users/${ownerId}/shops/${shopId}/financial_rollups_daily/${keys.dayKey}`), rollups, { merge: true });
    batch.set(db.doc(`users/${ownerId}/shops/${shopId}/financial_rollups_monthly/${keys.monthKey}`), rollupPayloadMerge(rollups), { merge: true });

    // =========================================================================
    // 🚀 DISTRIBUTED SHARDED COUNTER ENGINE (HOTSPOTTING CRASH ELIMINATED)
    // global_balance_sheet වෙනුවට Shards 10 කට ලිවීමෙන් 1 write/sec සීමාවෙන් පද්ධතිය බේරාගැනීම
    // =========================================================================
    const shardIndex = Math.floor(Math.random() * 10);
    const shardDocRef = db.doc(`users/${ownerId}/shops/${shopId}/balance_sheet_shards/shard_${shardIndex}`);
    batch.set(shardDocRef, gl, { merge: true });
