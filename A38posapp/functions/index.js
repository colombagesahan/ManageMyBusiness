...
async function assertAccountingPeriodUnlocked(tenantId, shopId, rawDate) {
    // 🛡️ ZERO-TRUST FAIL-CLOSED GUARD: Block evasion attempts using missing/corrupted timestamps
    if (!rawDate) {
        throw new HttpsError('invalid-argument', 'SECURITY REFUSAL: Missing authoritative transaction timestamp.');
    }
    let dateObj;
    ...
    if (isNaN(dateObj.getTime())) {
        throw new HttpsError('invalid-argument', 'SECURITY REFUSAL: Corrupted or unparseable transaction timestamp format.');
    }
...
exports.sendEnterpriseEmail = onCall(async (request) => {
    ...
    Powered by ${shopName} | Platform by <a href="https://www.applicaz.com" target="_blank" style="color: inherit; text-decoration: underline; font-weight: bold;">www.applicaz.com</a>
</span>
                    </div>
                </div>
            `;
...
async function getValidRamisToken(tenantId, shopId) {
    ...
    // =========================================================================
    const masterEncryptionKey = process.env.RAMIS_MASTER_KEY || "APPLICAZ_CORE_IRD_MASTER_KEY_2026";
...
exports.pushInvoiceToRamis = onCall({ timeoutSeconds: 30, memory: "512MiB" }, async (request) => {
    ...
    // 🏛️ ZERO-TRUST STATUTORY TEMPORAL AUDIT: Block IRD push if period is audited & closed
    await assertAccountingPeriodUnlocked(context.tenantId, context.shopId, verifiedSale.date);
...
exports.pushCreditDebitNoteToRamis = onCall({ timeoutSeconds: 30, memory: "512MiB" }, async (request) => {
    ...
    // 🏛️ ZERO-TRUST STATUTORY TEMPORAL AUDIT: Block credit/debit notes on closed periods
    await assertAccountingPeriodUnlocked(safeTenantId, safeShopId, date);
...
exports.onTransferWritten = onDocumentWritten("users/{ownerId}/transfers/{transferId}", async (event) => {
    ...
    // 🏛️ ZERO-TRUST TEMPORAL PERIOD-LOCK AUDIT ON BOTH BRANCHES (LKAS 8 / IRD COMPLIANCE)
    try {
        await assertAccountingPeriodUnlocked(ownerId, tr.sourceShopId, tr.date);
        await assertAccountingPeriodUnlocked(ownerId, tr.targetShopId, tr.date);
    } catch (periodErr) {
        const secureTime = new Date().toISOString();
        const alertMsg = `CRITICAL FRAUD BLOCKED: Transfer #${transferId} was backdated into officially CLOSED & AUDITED period! Transfer quarantined.`;
        if (event.data?.after?.ref) {
            await event.data.after.ref.update({
                rollupsProcessed: true,
                securityStatus: "FRAUD_BLOCKED",
                fraudDetails: alertMsg,
                periodLockedRejection: true
            });
        ...
        await db.collection(`users/${ownerId}/system_audit_logs`).add({
            timestamp: secureTime, userEmail: "ZERO_TRUST_PERIOD_GUARD", userRole: "system", shopId: tr.sourceShopId, shopName: "Security Module",
            action: "BACKDATED_TRANSFER_INTO_LOCKED_PERIOD_BLOCKED", description: alertMsg
        });
        return;
    }
...
exports.onIssuedChequeWritten = onDocumentWritten({
    ...
    if (!after) return; 
    // 🛡️ AMOUNT & STATUS CHANGE DETECTOR: Process if status changed OR if amount edited on pending cheque
    const isAmountChanged = before && after && secureRound(before.amount) !== secureRound(after.amount);
    if (before && before.status === after.status && !isAmountChanged) return; 

    // 🛡️ ZERO-TRUST IDEMPOTENCY GUARD: Check live document against retry double-booking
    if (!isAmountChanged && after.cqrsLastStatusProcessed === after.status) return;
    const liveDoc = await event.data.after.ref.get();
    if (!isAmountChanged && liveDoc.exists && liveDoc.data().cqrsLastStatusProcessed === after.status) {
...
exports.onProductWritten = onDocumentWritten("users/{ownerId}/shops/{shopId}/products/{productId}", async (event) => {
    ...
    const batch = db.batch();
    if (before && !after) {
        // 🏛️ ATOMIC TOMBSTONE GUARD: Atomic batch write protects against duplicate rollback on GCF retries
        const delMarkerRef = db.doc(`users/${event.params.ownerId}/shops/${event.params.shopId}/cqrs_events/del_prod_${event.params.productId}`);
        const delMarkerSnap = await delMarkerRef.get();
        if (delMarkerSnap.exists) {
            console.log(`[CQRS IDEMPOTENCY] Product deletion ${event.params.productId} already processed. Suppressing duplicate delete retry.`);
            return;
        }
        batch.set(delMarkerRef, { eventId: event.id, deletedAt: admin.firestore.FieldValue.serverTimestamp() });
    }
    ...
    if (assetDelta !== 0 || payablesDelta !== 0) {
        ...
        // Atomic commit for deletion tombstone even when asset delta is zero
        await batch.commit();
    }
});
// ------------------------------------------------------------------------
...
exports.underwriteCreditRisk = onCall({ timeoutSeconds: 15, memory: "256MiB" }, async (request) => {
    ...
    try {
        ...
        let hasIndexDegradation = false;
        const unpaidSalesPromise = salesRef
            .where("customer.phone", "==", cleanPhone)
            .where("creditBalance", ">", 0)
            .get()
            .catch((idxErr) => {
                console.warn(`[UNDERWRITING INDEX ALERT] Composite index unavailable for phone ${cleanPhone}:`, idxErr.message);
                hasIndexDegradation = true;
        ...
        if (isNewCustomer) {
            ...
            // 🛡️ ZERO-TRUST NULL-SAFETY: Safe data extraction against corrupted profiles
            const cData = custSnap.data() || {};
...
exports.generateBankUnderwritingReport = onCall({ timeoutSeconds: 30, memory: "512MiB" }, async (request) => {
    ...
    // 🚨 ATOMIC TRANSACTIONAL RATE LIMIT & PASSCODE VERIFICATION (TOCTOU RACE CONDITION IMMUNE)
    const verificationResult = await db.runTransaction(async (t) => {
        const rateSnap = await t.get(rateLimitRef);
        let failedAttempts = 0;
        ...
        const cSnap = await t.get(consentRef);
        ...
        const realPasscode = String(consent.passcode || '').trim().toUpperCase();

        // Constant-Time Comparison against timing side-channel attacks
        const userBuf = Buffer.from(enteredPasscode);
        const realBuf = Buffer.from(realPasscode);
        let isMatch = false;
        if (userBuf.length === realBuf.length && userBuf.length > 0) {
            isMatch = crypto.timingSafeEqual(userBuf, realBuf);
        }
        ...
        if (!isMatch) {
            ...
            if (nextFails >= 5) {
                lockoutTime = serverNowMs + (3600 * 1000); // 1-hour lockout
                const alertRef = db.collection(`users/${safeOwnerId}/shops/${safeShopId}/alerts`).doc();
                t.set(alertRef, {
                    refId: 'BANK_TELEMETRY_GATEWAY',
                    type: 'global',
                    orderId: 'BANK_AUTH_LOCKOUT',
                    message: `🚨 SECURITY BREACH: 5 Failed Bank Telemetry attempts. Locked for 1 hour!`,
                    targetDate: new Date(serverNowMs).toISOString(),
                    frequency: 'once', status: 'triggered', createdAt: new Date(serverNowMs).toISOString()
                });
            }
            ...
            t.set(rateLimitRef, {
                failedAttempts: nextFails >= 5 ? 0 : nextFails,
                lockedUntil: lockoutTime,
                lastFailedAt: serverNowMs
            }, { merge: true });

            return { isMatch: false, attemptsRemaining: Math.max(0, 5 - nextFails), isLocked: nextFails >= 5 };
        }

        // On Success: Reset in same transaction
        t.set(rateLimitRef, { failedAttempts: 0, lockedUntil: 0, lastSuccessAt: serverNowMs }, { merge: true });
        return { isMatch: true, consent: consent };
    });

    if (!verificationResult.isMatch) {
        if (verificationResult.isLocked) {
            throw new HttpsError('resource-exhausted', 'SECURITY LOCKOUT: Too many failed passcode attempts! Bank telemetry locked for 1 hour.');
        }
        throw new HttpsError('permission-denied', `SECURITY REFUSAL: Invalid Bank Telemetry Passcode. (${verificationResult.attemptsRemaining} attempts left)`);
    }

    const consent = verificationResult.consent;
    ...
    try {
        ...
        // 🏛️ ZERO-LOSS SME FALLBACK: If merchant is new and has no historical completed months, audit current Month-To-Date (MTD)
        if (monthsAudited === 0) {
            const currentMKey = `${slDateNow.getUTCFullYear()}_${String(slDateNow.getUTCMonth() + 1).padStart(2, '0')}`;
            const curShardsColRef = db.collection(`users/${safeOwnerId}/shops/${safeShopId}/financial_rollups_monthly/${currentMKey}/shards`);
            const curParentDocRef = db.doc(`users/${safeOwnerId}/shops/${safeShopId}/financial_rollups_monthly/${currentMKey}`);
            const [curShardsSnap, curParentSnap] = await Promise.all([curShardsColRef.get(), curParentDocRef.get()]);

            if (!curShardsSnap.empty || curParentSnap.exists) {
                const curMData = {
                    grossSales: 0, totalDiscounts: 0, taxCollected: 0, vatCollected: 0,
                    ssclCollected: 0, salesReturns: 0, taxReversed: 0, ssclReversed: 0,
                    totalCOGS: 0, operationalExpenses: 0, payrollExpenses: 0
                };
                if (curParentSnap.exists) {
                    const pd = curParentSnap.data();
                    for (const k of Object.keys(curMData)) if (pd[k] !== undefined) curMData[k] += (parseFloat(pd[k]) || 0);
                }
                curShardsSnap.forEach(sDoc => {
                    const sd = sDoc.data();
                    for (const k of Object.keys(curMData)) if (sd[k] !== undefined) curMData[k] += (parseFloat(sd[k]) || 0);
                });
                const effTaxCol = (curMData.vatCollected > 0 || curMData.ssclCollected > 0) ? (curMData.vatCollected + curMData.ssclCollected) : (curMData.taxCollected || 0);
                const effTaxRev = (curMData.taxReversed || 0) + (curMData.ssclReversed || 0);
                const curNetRev = Math.max(0, secureRound(curMData.grossSales - curMData.totalDiscounts - effTaxCol - (curMData.salesReturns - effTaxRev)));
                
                total6mRevenue += curNetRev;
                total6mCogs += Math.max(0, secureRound(curMData.totalCOGS));
                total6mOpex += Math.max(0, secureRound(curMData.operationalExpenses + curMData.payrollExpenses));
                monthsAudited = 1;
            }
        }
        ...
        // 5. Basel III Ratios Calculation (Reconciled with Issued PDCs & Unearned Store Credits)
        const quickAssets = liquidCashInHand + liquidCashAtBank + accountsReceivable;
        const quickLiabilities = accountsPayable + (consolidatedGl.pendingIssCheques || 0) + (consolidatedGl.storeCredits || 0) + taxLiabilities;
...
- Business: Merchant Store (Shop ID: ${safeShopId})
...
Paragraph 3: Executive Underwriting Recommendation (Recommended Loan Ceiling, Tenure, and Required Covenants).`;
        ...
        // 🛡️ FORENSIC REPRODUCIBILITY & ZERO-LEAK SANITIZED SEAL
        const generatedAtIso = new Date().toISOString();
        const rawSealString = `${safeShopId}_${avgMonthlyRevenue}_${avgMonthlyNetOperatingIncome}_${quickRatio}_${generatedAtIso}`;
        const digitalAuditSeal = crypto.createHash('sha256').update(rawSealString).digest('hex').toUpperCase();

        return {
            success: true,
            verifiedShopId: safeShopId,
...
exports.closeAccountingPeriod = onCall({ timeoutSeconds: 60, memory: "1GiB" }, async (request) => {
    ...
    const pnl = {
        ...
        vatCollected: 0, ssclCollected: 0, taxReversed: 0, ssclReversed: 0,
    ...
    // 🏛️ IFRS & LKAS 1 COMPREHENSIVE REVENUE: Deducting Net Sales Returns (Net of VAT & SSCL Reversals)
    const netSalesReturns = secureRound(pnl.salesReturns - pnl.taxReversed - pnl.ssclReversed);
    const netRevenue = secureRound(pnl.grossSales - pnl.totalDiscounts - pnl.vatCollected - pnl.ssclCollected - netSalesReturns);
    ...
    const snapshotData = {
        ...
        taxLiabilities: {
            ...
            ssclReversed: pnl.ssclReversed,
            netVatPayable: statutoryNetVatPayable, // 👈 100% IRD Legal: Zero Negative Tax
            netSsclPayable: Math.max(0, secureRound(pnl.ssclCollected - pnl.ssclReversed)), // 👈 Statutory SSCL Payable
