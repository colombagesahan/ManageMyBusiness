...
exports.onSaleCreated = onDocumentCreated("users/{ownerId}/shops/{shopId}/sales/{saleId}", async (event) => {
   ...
   // =========================================================================
    // 🏛️ ZERO-VARIANCE STATUTORY AUDIT ENGINE (TAX INCLUSIVE AWARENESS)
    // Tax-Inclusive නම් subtotal තුළ බදු අඩංගු බැවින් නැවත taxAmt එකතු නොකිරීම
    // =========================================================================
    ...
    // 2. සෘණ අගයන්ගෙන් සල්ලි සොරකම් කිරීම පරීක්ෂාව (Negative Value & Sub-Item Injection Firewall)
    if ((sale.total || 0) < 0 || (sale.subtotal || 0) < 0 || (sale.discountAmt || 0) < 0 || (sale.cashGiven || 0) < 0 || (sale.creditBalance || 0) < 0 || (sale.walletApplied || 0) < 0 || (sale.loyaltyRedeemed || 0) < 0) {
        isFraud = true;
        fraudReasons.push("Negative Values Injected: Attempted system theft via negative balances.");
    }

    // තනි තනි භාණ්ඩ තුළ සෘණ ප්‍රමාණ හෝ මිල ගණන් සඟවා ඇත්දැයි බැලීම (Item-Level Non-Negative Assertion)
    if (sale.items && Array.isArray(sale.items)) {
        for (let itm of sale.items) {
            if ((parseFloat(itm.qty) || 0) <= 0 || (parseFloat(itm.sell) || 0) < 0) {
                isFraud = true;
                fraudReasons.push(`Invalid Item Payload: Item '${itm.name}' has non-positive quantity (${itm.qty}) or negative price (${itm.sell}).`);
                break;
            }
        }
    }
    ...
    const rollupPayload = {
        ...
        // 🚨 ZERO-TRUST ACCOUNTING: Balance the equation by logging Loyalty as a Nested OPEX Map
    ...
    // =========================================================================
    // 🏛️ IFRS 15 CONTRACT LIABILITY OFFSET (BUG 54 FIX)
    // පාරිභෝගිකයා Store Credit මගින් බඩු මිලදී ගත් විට Balance Sheet එකේ වගකීම නිවැරදිව අඩු කිරීම
    // =========================================================================
...
exports.underwriteCreditRisk = onCall({ timeoutSeconds: 15, memory: "256MiB" }, async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Security Firewall: Terminal unauthenticated.');
    }

    const { ownerId, shopId, customerPhone, billTotal, requestedCredit } = request.data;
    if (!ownerId || !shopId || !customerPhone || requestedCredit === undefined) {
        throw new HttpsError('invalid-argument', 'Missing required underwriting parameters.');
    }
    ...
    try {
        ...
        const custRef = db.doc(`users/${ownerId}/shops/${shopId}/customers/${cleanPhone}`);
        const custSnap = await custRef.get();

        // 2. Fetch Historical Sales Records for this Customer (Chronological Audit)
        const salesRef = db.collection(`users/${ownerId}/shops/${shopId}/sales`);
