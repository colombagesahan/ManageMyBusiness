...
// =========================================================================
// 🛡️ ZERO-TRUST MULTI-TENANT IDENTITY, BOLA FIREWALL & PROTOTYPE SHIELD
// (ලක්ෂ 100,000ක කඩවල දත්ත එකිනෙක කාන්දු වීම 100% ක් වළක්වන විශ්වීය පලිහ)
// =========================================================================
const DANGEROUS_PROTOTYPE_KEYS = new Set(['__proto__', 'constructor', 'prototype', 'toString', 'valueOf']);
...
async function assertTenantContext(auth, requestedShopId) {
    ...
    if (userData.role === 'admin') {
        ...
        validatedShopId = requestedShopId || userData.primaryShop || 'main';
...
exports.processSecureWallet = onCall(async (request) => {
    ...
    const customerRef = db.doc(`users/${safeTenantId}/shops/${safeShopId}/customers/${customerPhone}`);
...
async function executeRamisInvoicePush(tenantId, shopId, payload) {
    ...
    // =========================================================================
    const normTaxableBase = Math.round(((parseFloat(payload.taxableBase) || (normGrandTotal - normVatAmt - (parseFloat(payload.exemptBase) || 0))) + Number.EPSILON) * 100) / 100;
    ...
    const processedLineItems = (payload.items || []).map((i, index) => {
        ...
        // Line Item එකෙහි Net Base එකක් තිබේ නම් එය ගනී, නැතහොත් ගණනය කරයි
        let lineNetAmt = 0;
        let lineNetUnitPrice = 0;

        if (i.netLineBase !== undefined && i.netUnitPrice !== undefined) {
            ...
            if (isExempt) {
                ...
                // Fallback tax exclusive decomposition (1.2095 composite)
                lineNetUnitPrice = Math.round((rawPrice / 1.2095 + Number.EPSILON) * 100) / 100;
                lineNetAmt = Math.round((lineQty * lineNetUnitPrice + Number.EPSILON) * 100) / 100;
            }
        }
...
exports.pushCreditDebitNoteToRamis = onCall({ timeoutSeconds: 30, memory: "512MiB" }, async (request) => {
    ...
    try {
        ...
        // Base Value without VAT (Excluding non-refundable SSCL from the VAT taxable base)
        ...
        const ramisPayload = {
            ...
            DateOfIssue: date,
            Reason: String(reason || 'Customer Return').substring(0, 100),
...
exports.onSaleCreated = onDocumentCreated({
    ...
    // =========================================================================
    // 🏛️ TRANSACTIONAL RAMIS OUTBOX DISPATCH (ZERO-STRANDED DATA GUARANTEE)
    // Rollups Update වන එම Atomic Batch එක තුළදීම RAMIS Outbox Task එක ජනනය කිරීම
    // =========================================================================
    const isVatApplicable = (sale.vatAmt && parseFloat(sale.vatAmt) > 0) || (sale.taxAmt && parseFloat(sale.taxAmt) > 0);
    ...
    batch.update(saleDocRef, {
        ...
        irdVerificationStatus: isVatApplicable ? "QUEUED_OUTBOX" : "EXEMPT_NO_VAT"
    });
